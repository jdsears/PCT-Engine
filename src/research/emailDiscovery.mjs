...
// the credit spend stays bounded and visible. An optional campaign aims the
// spend at one campaign's members, the force lever's companion: without it
// the global score ordering sends every credit to the data centre tail while
// a fresh pharma wave waits.
export async function discoverEmails({ limit = 10, campaign = null, log = () => {} } = {}) {
  ...
  const params = [Math.min(Math.max(1, limit), 50)];
  let scope = '';
  if (campaign) {
    params.push(campaign);
    scope = ` AND EXISTS (SELECT 1 FROM company_campaigns m WHERE m.company_id = c.id AND m.campaign = $${params.length})`;
  }
  const { rows } = await pool.query(
    `SELECT ct.id, ct.full_name, ct.linkedin_url, ct.email, ct.email_verified_at,
     ...
     WHERE ct.in_decision_orbit AND NOT ct.suppressed
       ...
       AND (ct.linkedin_url IS NOT NULL OR (ct.full_name IS NOT NULL AND c.domain IS NOT NULL))${scope}
