...
// The ICP filter. The definition is campaign data now, held in the campaign
// registry; this module is the shared scoring machinery that reads it. The
// exported ICP_CONFIG and WEIGHTS stay as the data centre campaign's values so
// anything holding a reference still resolves, and they read from the registry
// rather than a second copy that could drift.

const DC = requireCampaign('marwin_dc');
export const ICP_CONFIG = {
  minProjectSizeMW: DC.icp.thresholds.minProjectSizeMW,
  buildStages: DC.icp.buildStages,
  minContractValueGBP: DC.icp.thresholds.minContractValueGBP,
  companyTypes: DC.icp.companyTypes,
  ukOnly: DC.icp.thresholds.ukOnly,
};
...
export const WEIGHTS = DC.icp.weights;
...
function signalPoints(signals, def) {
  const types = new Set(def.icp.signalTypes);
  const dcSignals = (signals || []).filter(s => types.has(s.signal_type));
  if (dcSignals.length === 0) return { points: 0, reason: def.icp.noSignalReason };
...
export function scoreCompany(company, signals, contacts = null, campaign = 'marwin_dc') {
  const def = typeof campaign === 'string' ? requireCampaign(campaign) : campaign;
  const draft = CONTACTABILITY_DRAFT.enabled();
  const W = draft ? CONTACTABILITY_DRAFT.weights : def.icp.weights;
  const breakdown = {};
  ...
  breakdown.company_type = def.icp.companyTypes.includes(company.company_type)
  ...
  breakdown.signals = { ...signalPoints(signals, def), max: W.signals };
