...
//
// Since 24 August 2026 every section also splits by campaign, John's ask when
// the digest read as a data centre report with pharma invisible inside the
// totals: every number was a whole-engine aggregate from the single-campaign
// era, and the signals line hardcoded "data centre stories" over both lanes.
// The totals stay; each lane's share rides beside them, zeroes included, so a
// quiet campaign is visibly quiet rather than averaged away.
...
export async function gatherDigestData() {
  ...
  // The stories split by the campaign that swept them; the filings stay
  // register-wide, which the wording says, because the Companies House poll
  // watches the register as a whole.
  s.lanes = (await pool.query(
    `SELECT campaign, count(*)::int AS news FROM signals
     WHERE observed_at >= ${week} AND signal_type LIKE 'news%'
     GROUP BY campaign ORDER BY campaign`)).rows;
  ...
  l.lanes = (await pool.query(
    `SELECT campaign, count(*) FILTER (WHERE created_at >= ${week})::int AS created,
            count(*) FILTER (WHERE updated_at >= ${week} AND created_at < ${week})::int AS refreshed
     FROM leads WHERE campaign <> 'rehearsal' GROUP BY campaign ORDER BY campaign`)).rows;
  ...
  d.lanes = (await pool.query(
    `SELECT campaign, count(*) FILTER (WHERE status = 'draft')::int AS waiting,
            count(*) FILTER (WHERE created_at >= ${week})::int AS drafted_this_week
     FROM outbound_drafts WHERE campaign <> 'rehearsal' GROUP BY campaign ORDER BY campaign`)).rows;
  ...
  try {
    ...
    // Sends and replies per lane, merged on campaign so a lane with sends but
    // no replies still shows both figures.
    const sl = (await pool.query(
      `SELECT d.campaign, count(*)::int AS sent FROM outbound_sends s JOIN outbound_drafts d ON d.id = s.draft_id
       WHERE s.sent AND NOT s.test_mode AND d.campaign <> 'rehearsal' AND s.created_at >= ${week}
       GROUP BY d.campaign`)).rows;
    const rl = (await pool.query(
      `SELECT d.campaign, count(*)::int AS replies FROM outbound_replies r JOIN outbound_drafts d ON d.id = r.draft_id
       WHERE d.campaign <> 'rehearsal' AND r.created_at >= ${week} GROUP BY d.campaign`)).rows;
  ...
  try {
    ...
    if (reg) {
      ...
      // Each lane's queue, derived exactly as the studio derives it, so the
      // warning can name the lane whose slot will pass silently.
      posts.lanes = (await pool.query(
        `SELECT COALESCE(lp.grounding->>'campaign', s2.campaign, 'marwin_dc') AS campaign,
                count(*) FILTER (WHERE lp.status = 'draft')::int AS waiting,
                count(*) FILTER (WHERE lp.status = 'approved')::int AS approved,
                count(*) FILTER (WHERE lp.status = 'posted' AND lp.posted_at >= ${week})::int AS posted
  ...
  try {
    ...
    if (reg) {
      ...
      interest.lanes = (await pool.query(
        `SELECT campaign, count(*) FILTER (WHERE first_seen_at >= ${week})::int AS gathered,
                count(*) FILTER (WHERE status = 'new')::int AS waiting
  ...
  try {
    ...
    // A contact has no campaign of its own; the company's membership decides,
    // exactly one registered membership or the data centre default, the same
    // derivation the connect queue uses. The search count stays a total,
    // because the call ledger records no campaign, and a split that guessed
    // would be worse than a total that is true. Lanes with nothing this week
    // are dropped here: this split reports the week's activity, unlike the
...
// beside each total, labelled with the campaign's display name. Nothing
// renders unless the data names more than one lane, so a single-campaign
// digest and every old-shaped test fixture read exactly as before.
const laneLabel = id => getCampaign(id)?.displayName || String(id);
export function laneSplit(lanes, key) {
  if (!Array.isArray(lanes) || lanes.length < 2) return '';
  return ` (${lanes.map(x => `${laneLabel(x.campaign)} ${x[key] ?? 0}`).join(', ')})`;
}
...
// switch, no lanes) renders exactly as it used to. With lanes the warning
// names the lane whose slot will pass silently, since "the queue is empty"
// helped nobody decide whose queue to fill.
function studioLine(posts) {
  const line = `${posts.waiting} post draft${posts.waiting === 1 ? '' : 's'} waiting${laneSplit(posts.lanes, 'waiting')}`
    + (posts.approved != null ? `, ${posts.approved} approved in the queue${laneSplit(posts.lanes, 'approved')}` : '')
    + `, ${posts.posted} posted this week${laneSplit(posts.lanes, 'posted')}.`;
  let warn = '';
  if (posts.autoOn === false) warn = 'The studio autopilot is switched off; posts publish only by hand.';
  else if (posts.autoOn === true) {
    const empty = (posts.lanes || []).filter(x => (x.approved ?? 0) === 0);
    if (Array.isArray(posts.lanes) && posts.lanes.length > 1 && empty.length) {
      warn = `The approved queue is empty for ${empty.map(x => laneLabel(x.campaign)).join(' and ')}, so ${empty.length === 1 ? 'its' : 'their'} next posting slot will pass silently.`;
    } else if ((posts.approved ?? 0) === 0) {
      warn = 'The approved queue is empty, so the next posting slot will pass silently.';
    }
  }
  return { line, warn };
}
...
export function renderDigest(data, { weekEnding = new Date().toISOString().slice(0, 10) } = {}) {
  ...
  const lines = [
    ...
    '',
    `Signals: ${s.news} stor${s.news === 1 ? 'y' : 'ies'} kept${laneSplit(s.lanes, 'news')}, ${s.uk} UK project and ${s.watch} watchlist, ${s.filings} filing${s.filings === 1 ? '' : 's'} tracked across the whole register.`,
    `Leads: ${l.created} new${laneSplit(l.lanes, 'created')}, ${l.refreshed} refreshed${laneSplit(l.lanes, 'refreshed')}.`,
    `Outbound: ${d.waiting} draft${d.waiting === 1 ? '' : 's'} awaiting review${laneSplit(d.lanes, 'waiting')}, ${d.approved} approved, ${d.drafted_this_week} drafted this week${laneSplit(d.lanes, 'drafted_this_week')}. Nothing sends without a person.`,
