...
export function buildGateSystem(campaign) {
  ...
  return (
    ...
    // The title-only path was where promotional posts and roundups got in: the
    // primary-subject and roundup rules above read the body, so on thin content
    // they never bite, and a promotional title naming the sector passed. This
    // closes that path. It is shared scaffolding rather than campaign wording
    // because the genre is not sector-specific: a listicle of pharmaceutical
    // plants is the same failure as a listicle of data centres.
